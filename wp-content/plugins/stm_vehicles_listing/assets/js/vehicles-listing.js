(function($) {
	...
	$(document).ready(function ($) {
		...
		$('.stm_vehicles_listing_icons .inner .stm_font_nav a').click(function(e){
			e.preventDefault();
		...
		$(document).on('click', '.stm_vehicles_listing_row_actions a[href="#delete"]', function(e) {
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data: 'number=' + numberTr + '&action=stm_listings_delete_single_option_row',
				context: this,
				beforeSend: function () {
		...
		$(document).on('click', '.stm_vehicles_listing_row_actions a[href="#save"]', function() {
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data: $stmForm.serialize() + '&action=stm_listings_save_single_option_row',
				context: this,
				beforeSend: function () {
					$stmOptions.addClass('loading');
				},
		...
		$(stmSortableSelector).sortable({
			stop: function (event, ui) {
				...
				$.ajax({
					...
					type: 'POST',
					dataType: 'json',
					data: 'order=' + newOrder.join(',') + '&action=stm_listings_save_option_order',
					context: this,
					beforeSend: function () {
		...
		$(document).on('click', '.stm_vehicles_add_new .listing_categories_add_new .stm_vehicles_listing_option_meta ' +
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data: $stmForm.serialize() + '&action=stm_listings_add_new_option',
				context: this,
				beforeSend: function () {

	...
	<div id="payment">
		<?php if ( $available_gateways = WC()->payment_gateways->get_available_payment_gateways() ) : ?>
			<ul class="payment_methods methods"><?php
				...
				foreach ( $available_gateways as $gateway ) {
					...
					<li class="payment_method_<?php echo esc_attr($gateway->id); ?>">
						...
						<label for="payment_method_<?php echo esc_attr($gateway->id); ?>"><?php echo esc_attr($gateway->get_title()); ?> <?php echo balanceTags($gateway->get_icon()); ?></label>
						<?php
			...
			<div class="form-row">
				...
				<input type="submit" class="button alt" id="place_order" value="<?php esc_attr_e( 'Add Payment Method', 'autoshowroom' ); ?>" />
				<input type="hidden" name="woocommerce_add_payment_method" value="1" />
			</div>
		<?php else : ?>
			<p><?php esc_html_e( 'Sorry, it seems that there are no payment methods which support adding a new payment method. Please contact us if you require assistance or wish to make alternate arrangements.', 'autoshowroom' ); ?></p>
		<?php endif; ?>
	</div>
</form>

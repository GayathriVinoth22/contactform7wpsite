...
function stm_get_rental_order_fields_values( $empty = false )
{
    ...
    $pickup_date_calc = !empty( $_COOKIE[$fields[5]] ) ? sanitize_text_field( $_COOKIE[$fields[5]] ) : false;
    $return_date_calc = !empty( $_COOKIE[$fields[6]] ) ? sanitize_text_field( $_COOKIE[$fields[6]] ) : false;
...
function stm_remove_order_custom_post_meta( $order_id, $status )
{
...
add_action( 'woocommerce_order_edit_status', 'stm_remove_order_custom_post_meta', 100, 2 );
add_action( 'woocommerce_order_status_completed', 'stm_remove_order_custom_post_meta_hard', 100, 1 );
...
function stm_getInfoWindowPriceManip( $id )
{
    ...
    if ( !empty( $priceDate ) || !empty( $discount ) || !empty( $pricePerHour ) ) { ?>
